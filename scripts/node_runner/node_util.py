...
import py_utils
from py_utils import binary_manager, dependency_util
...
class _NodeManager(object):
  ...
  def InitNode(self):
    ...
    os.chdir(
      os.path.join(
        os.path.abspath(py_utils.GetCatapultDir()),
        'common', 'node_runner', 'node_runner'
      )
    )
...
def GetNodeModulesPath():
  ...
  if sys.platform.startswith('win'):
    # Escape path on Windows because it's very long and
    # must be passed to NTFS.
    path = u'\\\\?\\' + path
  return path
